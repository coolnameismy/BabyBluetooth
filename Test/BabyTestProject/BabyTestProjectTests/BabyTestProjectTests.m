...
- (void)testCentralModelMainOfDelegateAndFilter {
    ...
    //设置查找设备的过滤器
    [self.baby setFilterOnDiscoverPeripherals:^BOOL(NSString *peripheralName) {
        [filterOnDiscoverPeripheralsExp fulfill];
        return YES;
    }];
    
    //设置连接设备的过滤器
    [self.baby setFilterOnConnectToPeripherals:^BOOL(NSString *peripheralName) {
    ...
    //启动中心设备
    self.baby.scanForPeripherals().connectToPeripherals().discoverServices().discoverCharacteristics().discoverDescriptorsForCharacteristic().readValueForCharacteristic().readValueForDescriptors();
